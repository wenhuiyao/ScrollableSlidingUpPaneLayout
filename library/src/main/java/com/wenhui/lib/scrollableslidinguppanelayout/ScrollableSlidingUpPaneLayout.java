...
public class ScrollableSlidingUpPaneLayout extends ViewGroup {
	...
	private boolean mShouldDrawViewUnderneathPanel = false;
	 ...
	 */
	private boolean mPreservedExpandedState = true;
	...
	/**
	 * No-op stubs for {@link com.wenhui.lib.scrollableslidinguppanelayout.ScrollableSlidingUpPaneLayout.PanelSlideListener}. If you only want to implement
	...
	public ScrollableSlidingUpPaneLayout(Context context, AttributeSet attrs,
		...
		try{
			...
			mCanSlide = a.getBoolean(R.styleable.ScrollableSlidingUpPaneLayout_can_slide, true);
			mMiddleClampBound = a.getDimensionPixelSize(R.styleable.ScrollableSlidingUpPaneLayout_middle_clamp_bound, 0);
			mCoveredFadeColor = a.getColor(R.styleable.ScrollableSlidingUpPaneLayout_fade_color, DEFAULT_FADE_COLOR);
			mShouldDrawViewUnderneathPanel = a.getBoolean(R.styleable.ScrollableSlidingUpPaneLayout_draw_view_under_panel, false);
		} finally {
			a.recycle();
		}
	...
	public void setMiddleClampBound(int topBound) {
		this.mMiddleClampBound = topBound;
	}

	public int getMiddleClampBound() {
		return mMiddleClampBound;
	}
	...
	/**
	 * Enable scrollable view scroll when this layout reach upper edge
	 * @param view
	 */
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (mFirstLayout) {
			mMiddleClampYOffset = (float) (mMiddleClampBound)
					/ (getHeight() - mPanelHeight);
			mSlideOffset = mCanSlide && mPreservedExpandedState ? mMiddleClampYOffset
					: 1.f;
		}
	...
	private class DragHelperCallback extends ViewDragHelper.Callback {
		...
		public void onViewReleased(View releasedChild, float xvel, float yvel) {
			...
			if (yvel > 0) {
				if (mSlideOffset < mMiddleClampYOffset) {
					top += mMiddleClampBound;
				} else {
					top += mSlideRange;
				}
			} else if (yvel < 0) {
				if (mSlideOffset > (1f - mMiddleClampYOffset)) {
					top += mMiddleClampBound;
				}
			} else {
				float firstCheckPoint = mMiddleClampYOffset / 2f;
				float secondCheckPoint = (1f + mMiddleClampYOffset) / 2;
				if (mSlideOffset > firstCheckPoint
						&& mSlideOffset < secondCheckPoint) {
					top += mMiddleClampBound;
				} else if (mSlideOffset >= secondCheckPoint) {
					top += mSlideRange;
				}
			}
